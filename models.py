...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        return nn.AddBias(reluw2, self.b2)
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        h = nn.ReLU(h)
        for i, letter in enumerate(xs):
            if i != 0:
                first = nn.Linear(xs[i], self.w1)
                second = nn.Linear(h, self.wh)
                h = nn.Add(first, second)
                h = nn.ReLU(h)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                params = [self.w1, self.w2, self.wh]
                loss = self.get_loss(x, y)
                grad = nn.gradients(loss, params)
                self.w1.update(grad[0], -self.learning_rate)
                self.w2.update(grad[1], -self.learning_rate)
                self.wh.update(grad[2], -self.learning_rate)
